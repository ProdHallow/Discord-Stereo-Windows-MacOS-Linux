...
# Re-exec under bash if invoked via sh/zsh/dash
...
# Expected binary hash — offsets below are ONLY valid for this exact build
EXPECTED_MD5="f1295ce2f14beb330e77163d4d41be53"
EXPECTED_SIZE=51065824

# macOS/Clang offsets — valid ONLY for the binary above
OFFSET_CreateAudioFrameStereo=0xA1E34A
OFFSET_AudioEncoderOpusConfigSetChannels=0x41BE45
OFFSET_MonoDownmixer=0x9E9DEB
OFFSET_EmulateStereoSuccess1=0xA296FF
OFFSET_EmulateStereoSuccess2=0xA29700
OFFSET_EmulateBitrateModified=0xA29B5E
OFFSET_SetsBitrateBitrateValue=0x6091E0
OFFSET_SetsBitrateBitwiseOr=0x6091E8
OFFSET_Emulate48Khz=0xA29867
OFFSET_HighPassFilter=0x2B6EC0
OFFSET_HighpassCutoffFilter=0x403A90
OFFSET_DcReject=0x403C20
OFFSET_DownmixFunc=0x3FAE70
OFFSET_AudioEncoderOpusConfigIsOk=0x41C150
OFFSET_ThrowError=0x945240
OFFSET_DuplicateEmulateBitrateModified=0xA2EA44
OFFSET_EncoderConfigInit1=0x41BE4F
...
# macOS-safe file size formatting (no numfmt on macOS)
format_size() {
    local bytes="$1"
...
# macOS stat wrappers (BSD stat, not GNU)
...
# Force base-10 (avoids octal issues with 08/09)
...
find_discord_clients() {
    ...
    # discord_voice.node only exists in the per-user Application Support
    # directories, inside versioned module folders. App bundle paths like
    # /Applications/Discord.app/Contents/Resources only contain the .asar
    # app bundle, not native modules.
    local home="$HOME"
    local search_bases=(
    ...
    for i in "${!search_bases[@]}"; do
        ...
        # Find discord_voice.node inside versioned module directories
        local found_nodes
        ...
        if [[ -n "$latest" && -f "$latest" ]]; then
            ...
            CLIENT_NODES+=("$latest")
            local size
            size=$(get_file_size "$latest" 2>/dev/null || stat -c%s "$latest" 2>/dev/null || echo "?")
            log_ok "Found: $name"
    ...
    if [[ ${#CLIENT_NAMES[@]} -eq 0 ]]; then
        ...
        echo "  ~/Library/Application Support/discord/0.0.*/modules/discord_voice-*/discord_voice/"
        echo "  ~/Library/Application Support/discordcanary/0.0.*/modules/discord_voice-*/discord_voice/"
        echo "  ~/Library/Application Support/discordptb/0.0.*/modules/discord_voice-*/discord_voice/"
        echo ""
...
verify_binary() {
    ...
    local fsize
    fsize=$(get_file_size "$node_path" 2>/dev/null || stat -c%s "$node_path" 2>/dev/null || echo "0")
    ...
    # MD5 check (macOS has md5, Linux has md5sum)
    local actual_md5
    if command -v md5 &>/dev/null; then
        ...
        log_warn "No md5 or md5sum found, skipping hash verification"
        return 0
    fi
...
kill_discord() {
    ...
    # Force kill if still running, but exclude our own PID to avoid suicide
    local my_pid=$$
    local discord_pids
    discord_pids=$(pgrep -i "discord" 2>/dev/null || true)
...
backup_node() {
    ...
    # Prune old backups per client (keep 3 — ~150MB for a 50MB node)
    local count
    count=$(ls -1 "$BACKUP_DIR"/discord_voice.node."${sanitized}".*.backup 2>/dev/null | wc -l | tr -d ' ')
    if (( count > 3 )); then
...
restore_from_backup() {
    ...
    for i in "${!backups[@]}"; do
        local bk="${backups[$i]}"
        local bsize
        bsize=$(get_file_size "$bk" 2>/dev/null || stat -c%s "$bk" 2>/dev/null || echo "?")
        local bdate
        bdate=$(get_file_date "$bk" 2>/dev/null || stat -c%y "$bk" 2>/dev/null | cut -d. -f1 || echo "unknown")
...
main() {
    ...
    # Close Discord (needed on macOS — code signing prevents patching in-place)
    kill_discord
