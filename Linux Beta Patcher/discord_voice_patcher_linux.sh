...
# Linux/Clang offsets — Auto-generated by discord_voice_node_offset_finder.py v5.0
# Build: ELF binary | Size: 78740472 | MD5: 051ffb79353786dfbff06719ee0b8b16
...
find_discord_clients() {
    ...
    for i in "${!search_paths[@]}"; do
        ...
        # Pattern 1: config dir with app-*/modules/discord_voice*/discord_voice/
        local found_nodes
    ...
    if [[ ${#CLIENT_NAMES[@]} -eq 0 ]]; then
        ...
        echo ""
        echo "Make sure Discord is installed. If using a non-standard install,"
        echo "you can copy this script's output and manually specify the path."
        return 1
    fi
...
kill_discord() {
    ...
    local procs=("Discord" "DiscordCanary" "DiscordPTB" "discord" "discord-canary" "discord-ptb")
...
backup_node() {
    ...
    local count
    count=$(ls -1 "$BACKUP_DIR"/*.backup 2>/dev/null | wc -l)
    if (( count > 10 )); then
...
private:
    ...
    bool ApplyPatches(void* fileData, long long fileSize) {
        ...
        // EmulateStereoSuccess2: Linux has je (0x74), patch to jmp (0xEB)
        if (!PatchBytes(Offsets::EmulateStereoSuccess2, "\xEB", 1)) return false;
        // CreateAudioFrameStereo: Linux uses r12 not r13 — mov r12,rax; nop
        ...
        // HighPassFilter: just ret — ELF has no fixed image base, stub not needed
        ...
        // DownmixFunc: ret (was push rbp = 0x55 on Linux)
        if (!PatchBytes(Offsets::DownmixFunc, "\xC3", 1)) return false;
        // AudioEncoderOpusConfigIsOk: just ret (Clang prologue is push rbp)
...
PATCHEOF
    ...
    # Now substitute the offset values
    ...
    sed -i "s/OFFSET_CreateAudioFrameStereo/${OFFSET_CreateAudioFrameStereo}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_AudioEncoderOpusConfigSetChannels/${OFFSET_AudioEncoderOpusConfigSetChannels}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_MonoDownmixer/${OFFSET_MonoDownmixer}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_EmulateStereoSuccess1/${OFFSET_EmulateStereoSuccess1}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_EmulateStereoSuccess2/${OFFSET_EmulateStereoSuccess2}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_EmulateBitrateModified/${OFFSET_EmulateBitrateModified}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_SetsBitrateBitrateValue/${OFFSET_SetsBitrateBitrateValue}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_SetsBitrateBitwiseOr/${OFFSET_SetsBitrateBitwiseOr}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_Emulate48Khz/${OFFSET_Emulate48Khz}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_HighPassFilter/${OFFSET_HighPassFilter}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_HighpassCutoffFilter/${OFFSET_HighpassCutoffFilter}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_DcReject/${OFFSET_DcReject}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_DownmixFunc/${OFFSET_DownmixFunc}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_AudioEncoderOpusConfigIsOk/${OFFSET_AudioEncoderOpusConfigIsOk}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_ThrowError/${OFFSET_ThrowError}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_DuplicateEmulateBitrateModified/${OFFSET_DuplicateEmulateBitrateModified}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_EncoderConfigInit1/${OFFSET_EncoderConfigInit1}/g" "$TEMP_DIR/patcher.cpp"
    sed -i "s/OFFSET_EncoderConfigInit2/${OFFSET_EncoderConfigInit2}/g" "$TEMP_DIR/patcher.cpp"
...
main() {
    ...
    # Check for sudo/root if needed (many Discord installs are user-owned, so only warn)
    if [[ $EUID -ne 0 ]]; then
        log_info "Running as user $(whoami). If patching fails, try: sudo $0 $*"
    fi
    ...
    if (( selection == 255 )); then
        # Patch all (-1 wraps to 255 in bash return)
        total=${#CLIENT_NAMES[@]}
        for i in "${!CLIENT_NAMES[@]}"; do
